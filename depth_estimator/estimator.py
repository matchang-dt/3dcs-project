...
def inv_depth_estimate(cost_volume):
    """
    ...
    inv_depths = torch.arange(d, dtype=cost_volume.dtype, device=cost_volume.device) + 1
...
class DepthEstimator(L.LightningModule):
    ...
    def forward(self, cost_volume, images, features):
        ...
        inv_depth_map, depth_conf = inv_depth_estimate(cost_volume) # [B, K, H, W], [B, K, H, W]
