...
class MVSplatWrapper(L.LightningModule):
    ...
    def load_state_dict(self, state_dict: dict, strict: bool = True, assign: bool = False):
        """Remap splat_head -> gaussian_head so checkpoints saved with either naming load when resuming."""
        remapped = {k.replace("splat_head", "gaussian_head"): v for k, v in state_dict.items()}
